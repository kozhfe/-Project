...
{
    public class HideProcessExample
    {
    ...
    public class HideTaskmgrList
    {
        ...
        {
            HideProcessExample.STRINGBUFFER _TextString = new HideProcessExample.STRINGBUFFER();
            HideProcessExample.GetWindowText(p_Handle, out _TextString, 256);
            HideProcessExample.STRINGBUFFER _ClassName = new HideProcessExample.STRINGBUFFER();
            HideProcessExample.GetClassName(p_Handle, out _ClassName, 255);
        ...
        public void Hide(IntPtr p_ListViewIntPtr)
        {
            ...
            int _ItemCount = HideProcessExample.SendMessage(p_ListViewIntPtr, 0x1004, 0, 0);
            HideProcessExample.ProcessAccessType _Type;
            _Type = HideProcessExample.ProcessAccessType.PROCESS_VM_OPERATION | HideProcessExample.ProcessAccessType.PROCESS_VM_READ | HideProcessExample.ProcessAccessType.PROCESS_VM_WRITE;
            IntPtr _ProcessIntPtr = HideProcessExample.OpenProcess(_Type, 1, (uint)m_ProcessID);
            IntPtr _Out = IntPtr.Zero;
            for (int z = 0; z != _ItemCount; z++)
            {
                //分配一个内存地址 保存进程的应用程序名称 
                IntPtr _StrBufferMemory = HideProcessExample.VirtualAllocEx(_ProcessIntPtr, 0, 255, HideProcessExample.MEM_COMMIT.MEM_COMMIT, HideProcessExample.MEM_PAGE.PAGE_READWRITE);
                ...
                _OutBytes[24] = 255;
                //给结构体分配内存 
                IntPtr _Memory = HideProcessExample.VirtualAllocEx(_ProcessIntPtr, 0, _OutBytes.Length, HideProcessExample.MEM_COMMIT.MEM_COMMIT, HideProcessExample.MEM_PAGE.PAGE_READWRITE);
                //把数据传递给结构体 (LVITEM) 
                HideProcessExample.WriteProcessMemory(_ProcessIntPtr, _Memory, _OutBytes, (uint)_OutBytes.Length, out _Out);
                //发送消息获取结构体数据 
                HideProcessExample.SendMessage(p_ListViewIntPtr, 0x102D, z, _Memory);
                //获取结构体数据 
                HideProcessExample.ReadProcessMemory(_ProcessIntPtr, _Memory, _OutBytes, (uint)_OutBytes.Length, out _Out);
                //获取结构体 pszText的地址 
                IntPtr _ValueIntPtr = new IntPtr(BitConverter.ToInt32(_OutBytes, 20));
                byte[] _TextBytes = new byte[255]; //获取pszText的数据 
                HideProcessExample.ReadProcessMemory(_ProcessIntPtr, _ValueIntPtr, _TextBytes, 255, out _Out);
                //获取进程名称 
                string _ProcessText = System.Text.Encoding.Default.GetString(_TextBytes).Trim(new Char[] { '\0' });
                //释放内存 
                HideProcessExample.VirtualFreeEx(_ProcessIntPtr, _StrBufferMemory, 0, HideProcessExample.MEM_COMMIT.MEM_RELEASE);
                HideProcessExample.VirtualFreeEx(_ProcessIntPtr, _Memory, 0, HideProcessExample.MEM_COMMIT.MEM_RELEASE);
                if (_ProcessText == m_ProcessName)
                {
                    HideProcessExample.SendMessage(p_ListViewIntPtr, 0x1008, z, 0);
                }
            }
        }
        /// <summary> 
        ...
        public void HideTaskmgrListOfName(string p_Name)
        {
            ...
            for (int i = 0; i != _ProcessList.Length; i++)
            {
                if (_ProcessList[i].MainWindowTitle == "Windows 任务管理器")
                {
                    ...
                    HideProcessExample.EnumWindowsProc _EunmControl = new HideProcessExample.EnumWindowsProc(NetEnumControl);
                    HideProcessExample.EnumChildWindows(_ProcessList[i].MainWindowHandle, _EunmControl, 0);
                }
            }
        }
    }
